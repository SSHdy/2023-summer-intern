  ...
  <div id="rentManagement">
    <div class="query-box" id="queryBox">
      <div class="clearfix">
        ...
        <div class="query-item" style="margin-left:50px;">
          <div class="query-title">出租状态：</div>
          <div class="query-input">
            <el-select filterable v-model="rentState" placeholder="请选择出租状态" style="width:100%">
              <el-option
    ...
    <div class="table-box">
      ...
      >
        ...
        <el-table-column :show-overflow-tooltip="true" prop="lesseeUse" label="承租用途" align="center"></el-table-column>
    ...
    >
      ...
      <el-form :inline="true" :model="Form" :rules="rules" ref="Form">
        ...
        <el-form-item
          :label="operateType==='detail'? '付款周期：' : '付款周期'"
          label-width="120px"
          prop="paymentCycle"
          :class="{'detail-form-item' : operateType==='detail'}"
        >
          ...
          >
            <el-option
              v-for="item in paymentCycleOpts"
              :key="item.id"
              :label="item.text"
              :value="item.id"
            ></el-option>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        lesseeYears: [
          { required: true, message: "承租年限不能为空", trigger: "blur" }
        ],
        soilList: [
          { required: true, message: "承租位置不能为空", trigger: "change" }
        ],
        hasRentSite: [
  ...
  methods: {
    ...
    getList() {
      ...
      getRentInfoList(this.currentPage, this.pageSize, this.renter, this.rentState, 1)
        .then(res => {
    ...
    // 单个删除
    singleDetle(row) {
      this.$confirm(`此操作将永久删除租用单位为${row.renter}的出租信息, 是否继续?`, "提示", {
        ...
        .then(() => {
          deleteRent(parseInt(row.rentId))
            ...
            .catch(err => {
              const msg = err.result ? err.result : '删除出租信息失败！';
              this.$message({
                message: msg,
                type: "error"
              });
    ...
    singleUpdate(row) {
      ...
      getRentDetail(row.rentId).then(res => {
        ...
        for ([key, value] of Object.entries(res)) {
          if (this.Form[key] !== undefined && value !== '') this.Form[key] = value;
        }
        if (res.paymentTime) {
          this.Form.firstPaymentTime = res.paymentTime;
        }
        this.updateHouseId = [];
    ...
    singleDetail(row) {
      ...
      getRentDetail(row.rentId).then(res => {
        ...
        if (res.paymentTime) {
          this.Form.firstPaymentTime = moment(res.paymentTime).format('YYYY-MM-DD')
        } else {
    ...
    getSpareRoomTree(id, rentRelationshipIdList) {
      ...
      getSpareRoomTree(_id, 2, rentRelationshipIdList).then(res => {
        this.treeData = res;
      }).catch(err => {
      ...
      this.$confirm("退租, 是否继续?", "提示", {
        ...
        .then(() => {
          rentWithdrawal(parseInt(row.rentId))
            ...
            .catch(err => {
              const msg = err.result ? err.result : '退租失败';
              this.$message({
                message: msg,
                type: "error"
              });
...
<style>
#rentManagement .query-box {
  margin-bottom: 20px;
}
#rentManagement .query-item {
  font-size: 16px;
  height: 32px;
  line-height: 32px;
  float: left;
...
#rentManagement .query-item > div {
  float: left;
}
#rentManagement .query-item > .query-input {
  width: calc(100% - 80px);
}
#rentManagement .query-item .el-date-editor.el-input,
#rentManagement .query-item .el-input__inner {
  height: 32px;
  line-height: 32px;
  width: 100%;
}
#rentManagement .query-btn {
  float: left;
  margin-left: 50px;
}
#rentManagement .el-textarea__inner {
  width: 868px;
  line-height: 40px;
  padding-top: 0;
}
#rentManagement .el-textarea {
  vertical-align: middle !important;
}
#rentManagement .table-operate-box {
  width: 100%;
  height: 66px;
  background: #d9eef6;
  position: relative;
}
#rentManagement .operate-btn {
  position: absolute;
  left: 19px;
  top: 17px;
}
#rentManagement .operate-pager {
  position: absolute;
  right: 19px;
  top: 17px;
}
#rentManagement .upload_box .el-upload--picture-card {
  width: 100px;
  height: 100px;
  line-height: 100px;
}
...
#rentManagement .el-upload--picture-card {
  margin-bottom: 8px;
}
#rentManagement .el-upload-list > li {
  margin-bottom: 0;
}
#rentManagement .detail-form-item .el-input__inner {
  border: 0;
  padding-left: 0;
  width: 215px;
}
#rentManagement .detail-form-item .el-textarea__inner {
  border: 0;
  padding-left: 0;
  width: 550px;
}
#rentManagement .img-preview-item {
  float: left;
  margin-right: 20px;
  margin-bottom: 15px;
  width: 100px;
...
#rentManagement .img-preview-item > img {
  width: 100%;
  height: 100%;
}
#rentManagement .img-preview-item > .bigger-view-warpper {
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
#rentManagement .img-preview-item > .bigger-view-warpper > i {
  position: absolute;
  font-size: 26px;
  color: #ffffff;
  cursor: pointer;
...
#rentManagement .img-preview-item > .bigger-view-warpper > i:hover {
  color: #3891e0;
}
#rentManagement .img-preview-item > .bigger-view-warpper:hover > i {
  display: block;
}
#rentManagement .img-preview-item > .el-checkbox {
  position: absolute;
  bottom: -13px;
  right: 0;
}
#rentManagement .download-btn {
  padding: 7px 22px;
}
#rentManagement .img-preview-bigger {
  width: 100%;
  height: 100%;
  position: fixed;
  z-index: 100000;
...
#rentManagement .img-preview-bigger > .img-preview-bigger-mask {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
#rentManagement .img-preview-bigger > img {
  position: absolute;
  left: 0;
  right: 0;
  top: 0;
...
#rentManagement .img-preview-bigger > i {
  position: absolute;
  font-size: 50px;
  top: 30px;
  right: 30px;
...
#rentManagement .img-preview-bigger > i:hover {
  color: #3891e0;
}
#rentManagement .rent-site .el-input__inner {
  width: 868px;
}
#rentManagement .cert-preview-item .el-checkbox {
  margin-right: 5px;
  color: #606266;
}
#rentManagement .officeBuilding-form-item,
#rentManagement .state-form-item {
  width: 215px;
