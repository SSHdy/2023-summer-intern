  ...
  <div id="disposeQuery">
    ...
    >
      ...
      <div class="dialog-content">
        ...
        <div class="initApply-info-box" v-if="isShow.initApplyDb">
          <div class="content-item">
            <span>处置方式：</span>
            <span>{{initApplyDb.handleWay}}</span>
          </div>
          <div class="content-item">
            <span>初审人：</span>
          ...
          <div class="content-item">
            <span>初审时间：</span>
            <span>{{initApplyDb.initApplyTime}}</span>
          </div>
          <div class="content-item" style="width:100%;">
            <span>处置意见：</span>
        ...
        <div class="finance-info-box" v-if="isShow.financeApprovalDb">
          <div class="content-item">
            <span>审批人：</span>
            <span>{{financeApprovalDb.falUser}}</span>
          </div>
          <div class="content-item">
            <span>审批时间：</span>
            <span>{{financeApprovalDb.falTime}}</span>
          </div>
          <div class="content-item">
            <span>处置方式：</span>
            <span>{{financeApprovalDb.falAuditState === 1 ? '同意处置' : '不同意处置'}}</span>
          </div>
          <div class="content-item" style="width:100%;">
            <span>审批意见：</span>
            <span>{{financeApprovalDb.falComment ? financeApprovalDb.falComment : '暂无'}}</span>
          </div>
          <div
            class="content-item"
            style="width:100%;"
            v-show="financeApprovalDb.falApprovalData && financeApprovalDb.falApprovalData.length > 0"
          >
            <span>审批资料：</span>
            <div class="file-download">
              <el-checkbox-group
                v-model="financeDownLoadCheckedList"
                @change="financeCheckedFileChange"
              >
                <div
                  v-for="(file, index) in financeApprovalDb.falApprovalData"
                  :key="index"
              ...
              <el-button
                size="small"
                type="primary"
                @click="fileDownLoad(financeDownloadList, 1)"
              >点击下载文件</el-button>
            </div>
          </div>
        </div>
...
import { uploadUrl, getUserUnitList } from "@/axios/commonApi";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getHandleList(this.currentPage, this.pageSize, this.houseIdList, this.yardIdList, this.startTime, this.endTime, this.unitId)
        .then(res => {
    ...
    singleDetail(row) {
      ...
      leaderDetail(row.handleId).then(res => {
        ...
        // 绑定初审信息
        if (allInfo.initApplyDb !== undefined) {
          for ([key, value] of Object.entries(allInfo.initApplyDb)) {
            if (this.initApplyDb[key] !== undefined) {
              this.initApplyDb[key] = value;
            }
          }
          this.initApplyDb.handleWay = this.getHandleWay(this.initApplyDb.handleWay);
        }
        // 绑定审批信息
        if (allInfo.financeApprovalDb !== undefined) {
          for ([key, value] of Object.entries(allInfo.financeApprovalDb)) {
            if (this.financeApprovalDb[key] !== undefined) {
              this.financeApprovalDb[key] = value;
            }
          }
        }
        // 绑定拆除信息
    ...
    // 改变下载的checkbox
    financeCheckedFileChange(val) {
      this.financeDownloadList = [];
      this.financeDownLoadCheckedList.forEach(item => {
        const arr = item.split(',');
        const obj = {
          address: arr[0],
          fileName: arr[1]
        }
        this.financeDownloadList.push(obj);
      })
    },
    ...
    // dialog关闭的回调
    dialogClosed() {
      this.financeDownLoadCheckedList = [];
      this.financeDownloadList = [];
      ...
      this.clearForm(this.financeApprovalDb);
...
<style>
#disposeQuery .query-box {
  margin-bottom: 20px;
}
#disposeQuery .query-item {
  font-size: 16px;
  height: 32px;
  line-height: 32px;
  float: left;
...
#disposeQuery .query-item > div {
  float: left;
}
#disposeQuery .query-item > .query-input {
  width: calc(100% - 80px);
}
#disposeQuery .query-item .el-select {
  height: 32px;
  line-height: 32px;
  width: 100%;
}
#disposeQuery .query-item .el-date-editor.el-input,
#disposeQuery .query-item .el-input__inner {
  height: 32px;
  line-height: 32px;
  width: 100%;
}
#disposeQuery .query-btn {
  float: right;
}
#disposeQuery .table-operate-box {
  width: 100%;
  height: 66px;
  background: #d9eef6;
  position: relative;
}
#disposeQuery .operate-btn {
  position: absolute;
  left: 19px;
  top: 17px;
}
#disposeQuery .operate-pager {
  position: absolute;
  right: 19px;
  top: 17px;
}
#disposeQuery .content-item {
  margin-bottom: 22px;
  overflow: hidden;
  width: 50%;
  float: left;
}
#disposeQuery .content-item > span {
  display: block;
  height: 20px;
  line-height: 20px;
  float: left;
...
#disposeQuery .content-item > span:first-child {
  text-align: right;
  width: 108px;
}
#disposeQuery .content-item > span:last-child {
  width: calc(100% - 108px);
}
#disposeQuery .dialog-content > div {
  overflow: hidden;
  margin-bottom: 30px;
}
#disposeQuery .file-download {
  float: left;
}
#disposeQuery .file-download-item {
  margin-bottom: 22px;
}
</style>
