  ...
  methods: {
    ...
    handleCheckChange(data, obj) {
      ...
      if (isChecked) {
        ...
        fatherNodeId = parseInt(data.pid);
        if (
      ...
      let currentCheckedNodes = this.$refs[this.treeName].getCheckedNodes();
      let arr = []
      currentCheckedNodes.forEach((item,index)=>{
        if(item.type == 1){
          arr.push(index)
        }
      })
      if(arr.length>1){
        currentCheckedNodes = currentCheckedNodes.slice(0,arr[1])
      }
      ...
      if (currentCheckedNodes.length > 0) {
        for (let i = 0; i < currentCheckedNodes.length; i++) {
          if (currentCheckedNodes[i].type === 2) {
      ...
      for (let i = 0; i < this.oldCheckedNodes.length; i++) {
        if (this.oldCheckedNodes[i].type === 1) {
    ...
    eventBus.$on(
      ...
      (nameList, checkedIdList, expandIdList) => {
        ...
        setTimeout(() => {
          ...
          this.checkedfatherNodeId = expandIdList[1]
        }, 100);
      }
    );
